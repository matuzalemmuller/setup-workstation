...
# Based on https://raw.githubusercontent.com/laurent22/joplin/dev/Joplin_install_and_update.sh
# Modified to use ~/Programs as installation folder
...
if [[ -e ~/Programs/joplin/VERSION ]] && [[ $(< ~/Programs/joplin/VERSION) == "${RELEASE_VERSION}" ]]; then
    ...
    [[ -e ~/Programs/joplin/VERSION ]] && CURRENT_VERSION=$(< ~/Programs/joplin/VERSION)
...
DOWNLOAD_TYPE="New"
if [[ -f ~/Programs/joplin/Joplin.AppImage ]]; then
...
rm -f ~/Programs/joplin/*.AppImage ~/.local/share/applications/joplin.desktop ~/Programs/joplin/VERSION

# Creates the folder where the binary will be stored
mkdir -p ~/Programs/joplin/

# Download the latest version
mv "${TEMP_DIR}/Joplin.AppImage" ~/Programs/joplin/Joplin.AppImage

# Gives execution privileges
chmod +x ~/Programs/joplin/Joplin.AppImage
...
then
	...
	Exec=${HOME}/Programs/joplin/Joplin.AppImage ${SANDBOXPARAM} %u
	Icon=joplin
...
# Record version
echo "$RELEASE_VERSION" > ~/Programs/joplin/VERSION
